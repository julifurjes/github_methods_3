...
scenario: the condition of the experiment
attitude: whether they had to speak in formal or informal style
total_duration: the length of the time they were speaking
f0mn: our baseline
hiss_count: the amount of air sucking through the teeth
...
df_F1$scenario_factor <- as.factor(df_F1$scenario)

lm_as_factor <- lm(f0mn ~ scenario_factor, data=df_F1)
lm_as_factor

lm_as_integer <- lm(f0mn ~ scenario, data=df_F1)
lm_as_integer
```
    ...
    Factors interpret it as a finite amount of options, therefore it uses a true-false logic, while integer handles them as infinite amount of options, therefore as actual values.
    
```{r}
matrix_factor <- model.matrix(lm_as_factor)
matrix_factor

matrix_integer <- model.matrix(lm_as_integer)
matrix_integer
```
    
    ii. Which coding of _scenario_, as a factor or not, is more fitting?
    
    The integer misrepresents our data, it handles it wrongly.
...
```
  ...
  Males are much lower than females. Some of them aren't really affected by the attitude, but a few of them are. However, the scenario can affect them.
...
model2 <- lmer(f0mn ~ gender + (scenario | subject), data=politeness)
model2
...
```
    ...
    model4 has the lowest residual standard deviation (with the 2 intercept)
    
    model4 has the lowest AIC as well
    
```{r}
AIC1 <- AIC(logLik(model1))
AIC1
AIC2 <- AIC(logLik(model2))
AIC2
AIC3 <- AIC(logLik(model3))
AIC3
AIC4 <- AIC(logLik(model4))
AIC4
```
...
#checking the variance explained with R-squared
r.squaredGLMM(model2)
r.squaredGLMM(model3)
r.squaredGLMM(model4)
```
    
    model4 explains the most variance
...
```{r}
df3 <- lm(mean_f0mn ~ gender, data = df_3variables)
df3
```
...
```{r}
qqnorm(residuals(df3))
qqline(residuals(df3))

qqnorm(residuals(model1))
qqline(residuals(model1))
```
    model1 is better, because it has more data
