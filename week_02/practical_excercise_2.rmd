...
model_2_6 <- lmer(f0mn ~ gender + (1 | scenario) + (1 | subject), data = politeness)

qqnorm(residuals(model_2_6))
qqline(residuals(model_2_6))
...
3)  Choose the model that you think describe the data the best - and write a short report on the main findings based on this model. At least include the following:
  ...
  - males have lower pitch and politeness has also lower pitch
  
  iii. motivate why you would include separate intercepts for subjects and scenarios (if you think they should be included)
  
  - we have separate due to individual baseline (subject) and due to the fact that the scenarios can differ a lot even in the same conditions
